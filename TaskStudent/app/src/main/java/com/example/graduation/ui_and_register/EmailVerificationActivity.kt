...
class EmailVerificationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.email_verify_code)
    ...
    private fun verifyEmailCode(code: String) {
        ...
        com.example.graduation.ui_and_register.RetrofitClient.instance.verifyEmail(request)
            .enqueue(object : Callback<VerifyResponse> {
                override fun onResponse(call: Call<VerifyResponse>, response: Response<VerifyResponse>) {
                    if (response.isSuccessful && response.body()?.success == true) {
                        navigateToRegister()
                    } else {
                        Toast.makeText(this@EmailVerificationActivity, "Verification failed: ${response.body()?.message}", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onFailure(call: Call<VerifyResponse>, t: Throwable) {
                    Toast.makeText(this@EmailVerificationActivity, "API call failed: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            })
    }
